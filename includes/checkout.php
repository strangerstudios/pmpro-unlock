 ...
 */
function pmpro_unlock_add_wallet_to_checkout() {
    global $pmpro_level;
    $level_id = $pmpro_level->id;

    $wallet = pmpro_unlock_check_save_wallet(); // Get and save the wallet possibly.
    ...
    if ( is_wp_error( $wallet ) || ! $wallet ) {
        pmpro_unlock_connect_wallet_button();
    } else {
        
        // Let's check the validate lock status.
        $check_lock = pmpro_unlock_has_lock_access( $level_lock_options['network_rpc'], $level_lock_options['lock_address'], $wallet );
        ...
        if ( $check_lock ) {
            ...
            $checkout_url = pmpro_unlock_get_checkout_url( $level_lock_options, $redirect_uri );
...
add_action( 'pmpro_checkout_after_pricing_fields', 'pmpro_unlock_add_wallet_to_checkout' );
 ...
 *
 */
function pmpro_unlock_save_wallet_after_level_change( $level_id, $user_id, $cancel_level ) {
    // Try to save user's wallet after they're given a level.
    pmpro_unlock_check_save_wallet( $user_id );
    pmpro_unset_session_var( 'pmproup_code' ); // Remove any session VAR that may be there.
}
add_action( 'pmpro_after_change_membership_level', 'pmpro_unlock_save_wallet_after_level_change', 10, 3 );
 ...
 */
function pmpro_unlock_checkout_level( $checkout_level ) {
    ...
    if ( ! pmpro_unlock_has_lock_access( $level_lock_options['network_rpc'], $level_lock_options['lock_address'], $wallet ) ) {
        return $checkout_level;
    }
...
add_filter( 'pmpro_checkout_level', 'pmpro_unlock_checkout_level', 10, 1 );
 ...
 */
function pmpro_unlock_registration_checks( $continue ) {
    global $pmpro_level;
    ...
    // Let's see if they have access to the lock now.
    $continue = pmpro_unlock_has_lock_access( $level_lock_options['network_rpc'], $level_lock_options['lock_address'], $wallet );
    
    if ( ! $continue ) {
        pmpro_setMessage( esc_html__( 'You need an NFT to claim this membership', 'pmpro-unlock' ), 'pmpro_error' ); // Change this.
    } else {
        pmpro_unlock_clear_transients( $level_lock_options['lock_address'], $wallet );
    }

    return $continue;
}
add_filter( 'pmpro_registration_checks', 'pmpro_unlock_registration_checks' );
 ...
 */
function pmpro_unlock_after_checkout( $user_id, $morder ) {
    global $pmpro_level;
...
add_action( 'pmpro_after_checkout', 'pmpro_unlock_after_checkout', 10, 2 );
 ...
 */
function pmpro_unlock_after_cancel_membership_level( $level_id, $user_id, $cancel_level_id ) {
    // Remove the NFT ID from the user's meta.
    delete_user_meta( $user_id, 'pmproup_claimed_nft_' . $cancel_level_id );
}
add_action( 'pmpro_after_change_membership_level', 'pmpro_unlock_after_cancel_membership_level', 10, 3 );